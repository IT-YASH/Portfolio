...
router.get("/dashboard-project", middleware, async (req, res) => {
  try {
    ...
    res.render("project/dashboard-project", {
      local,
      data,
...
router.post("/add-project", async (req, res) => {
  try {
    ...
    const newPost = new project({
      Project_title: title,
      Project_body: body,
      createdAt: new Date(), // Set createdAt date to current date/time
      updatedAt: new Date(), // Set updatedAt date to current date/time
    });
    await newPost.save();
    res.redirect("/dashboard-project");
  } catch (error) {
